...
import kinoko.database.DatabaseConnection;
import kinoko.database.MemoAccessor;
import kinoko.database.table.MemoTable;
...
public class MysqlMemoAccessor implements MemoAccessor {
    private Memo loadMemo(ResultSet rs) {
        try {
            final MemoType type = kinoko.server.memo.MemoType.getByValue(rs.getInt(MemoTable.MEMO_TYPE));
            return new Memo(
                    type != null ? type : kinoko.server.memo.MemoType.DEFAULT,
